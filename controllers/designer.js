...
exports.getIndexJSON = function (req, res, next) {
  ...
  Design.find(findQuery)
    ...
    .populate({
      path: 'revisions',
      select: 'name snapshot store case fileID suggestCaseMode createdAt',
      populate: [{
        path: 'store',
        model: 'Store',
        select: 'ezID name'
...
exports.getRevisionsByFileId = function (req, res) {
  req.params.fileId = req.params.fileId.split(',')
  Revision.find({
    fileID: { $in: req.params.fileId }
  })
    .populate('store', '-cases -prices')
    .populate('case')
      ...
      if (store) {
        ...
        for (let index = 0; index < data.length; index++) {
          if (data[index].isCustom) {
            let obj = { isCustom: true, currentRev: data[index] }
            customFoam.push(obj)
          } else {
            dg.push(data[index].id)
          }
        }
            ...
            .exec(function (err, design) {
              if (design) {
                ...
                for (let index = 0; index < design.length; index++) {
                  const element = design[index].currentRev.id;
                  let selectedRevIndex = store.findIndex(item => item.id === element)
                  if (selectedRevIndex > -1) {
                    design[index].currentRev = store[selectedRevIndex]
                  }
                }
                let designs = design
                if (designs && designs.length > 0) {
                  designs = designs.concat(customFoam)
                } else {
                  designs = customFoam
                }
    ...
    .exec(function (err, sentDesign) {
      ...
      function findRevision(design) {
        Revision.findOne({
          ...
          if (req.body.hasOwnProperty('suggestCaseMode')) {
            rev.suggestCaseMode = req.body.suggestCaseMode;
          }
...
exports.postDesignJSON = function (req, res, next) {
  ...
  var revision = new Revision({
    ...
    name: 'design1',
    suggestCaseMode: req.body.suggestCaseMode,
    unit: req.body.unit,
    suggestCaseMode: req.body.suggestCaseMode,
